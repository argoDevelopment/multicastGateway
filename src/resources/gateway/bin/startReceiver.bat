...
REM
REM -dnr -> if this switch is set then "do not repeat" the traffic onto the multicast group
REM -ma -> (required) The multicast group to listen to join.  The standard Argo group is 230.0.0.1 
...
java -cp @INSTALL_DIR@/gateway/lib/@JAR_NAME@.jar -Djava.util.logging.config.file="@INSTALL_DIR@/gateway/bin/logging.properties" -Dnet.java.preferIPv4Stack=true ws.argo.mcg.GatewayReceiver %*
